...
Default to **Core (recommended for new users)** — copy the chosen skill folders from `.cursor/skills/` (this repo’s Cursor surface). For **Claude Code** installs only, a parallel `.agents/skills/*` tree may exist upstream in ECC; this hci-lab workspace is Cursor-first and does not vendor `.agents`. Add `skills/search-first/` when present for research-first workflows. Core covers engineering, evals, verification, security, strategic compaction, frontend design, and cross-functional skills (e.g. article-writing, content-engine, market-research, frontend-slides).
